...
// Motor data
int motor_ctrl_PWM_arr[arr_len];    // User customized PWM value
int motor_left_PWM_arr[arr_len];    // Final PWM value given to the left motor
int motor_right_PWM_arr[arr_len];   // Final PWM value given to the right motor
...
void data_save() {   // save the data in arrays
  if (arr_counter < arr_len) {
    ...
    motor_left_PWM_arr[arr_counter] = motor_left_PWM;
    motor_right_PWM_arr[arr_counter] = motor_right_PWM;

    // Store the ToF data
    ToF1_reading_arr[arr_counter] = ToF1_second;
    ToF2_reading_arr[arr_counter] = ToF2_second;
...
void data_arrays_reset() {   
  for (int i=0; i<arr_len; i++){
    ...
    motor_left_PWM_arr[i] = 0;
    motor_right_PWM_arr[i] = 0;
    ToF1_reading_arr[i] = 0;
    ToF1_extrapolate_arr[i] = 0;
    ToF2_reading_arr[i] = 0;
    p_ctrl_arr[i] = 0;
    i_ctrl_arr[i] = 0;
    d_ctrl_arr[i] = 0;
    pid_ctrl_arr[i] = 0;
