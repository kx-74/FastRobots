...
bool start_mapping;              // Flag to start the robot
...
int map_counter;                 // Keeps track of how many readings have been taken
...
void handle_command() {   
  ...
  switch (cmd_type) {        
    ...
    /*
      * Start mapping the room with ToF data
      */
    case START_MAPPING:
      data_arrays_reset();
      ...
      // Start the PID Orientation control
      Serial.println("Start orientation PID controller...");
      gyr_yaw = 0;    // reset the yaw angle for initial position
      ...
      ref_angle = 0;
      pid_ctrl = 0;
      map_counter = 1;    // reset the counter for counting angle increment numbers
    ...
    case GET_DATA_MAP:
      ...
      for (int i=0; i<arr_counter; i++) {
        ...
        tx_estring_value.append(pid_error_arr[i]);
        tx_estring_value.append(", ");
        tx_estring_value.append(ToF2_reading_arr[i]);
        tx_estring_value.append(", ");
        tx_estring_value.append(timestamp_arr[i]);
        tx_characteristic_string.writeValue(tx_estring_value.c_str());
...
void loop() {
  ...
  if (central) {
    ...
    while (central.connected()) {
      ...
      if (start_manual_motor_ctrl && !start_ToF_pid && !start_mapping && (millis()-start_time)<20000) {
        current_time = millis();

        // Combined with Orientation control
      ...
      // Make an in-axis turn, map out the room with ToF data
      if (start_mapping && !start_manual_motor_ctrl && !start_ToF_pid && (millis()-start_time)<20000){
        current_time = millis();

        // Get the ToF data and keep the history data
        if (distanceSensor1.checkForDataReady() && distanceSensor0.checkForDataReady()) {
          ToF1_first = ToF1_second;
          ToF2_first = ToF2_second;

          ToF1_second = distanceSensor1.getDistance();  // Front ToF sensor
          ToF2_second = distanceSensor0.getDistance();  // Side ToF sensor
        }

        // Delay 1 seconds before moving to the next orientation
        if ((current_time-start_time) > 1000*map_counter) { 
          // Average the current ToF reading and the previous one to reduce the error impacts
          ToF1_second = (ToF1_second + ToF1_first)/2;
          ToF2_second = (ToF2_second + ToF2_first)/2;
          data_save();
        ...
        // Stop the robot after a 360-degree turn
        if (ref_angle > 375) {
          start_IMU_pid = false;
          start_mapping = false;
        }
      ...
      if ((!start_manual_motor_ctrl) && (!start_IMU_pid) && (!start_ToF_pid) && (!start_mapping) || ((millis()-start_time)>=20000)) {
