...
int ToF0_arr_counter = 0;
int ToF1_arr[arr_len];
int ToF1_timestamp_arr[arr_len];
int ToF1_arr_counter = 0;
    ...
    switch (cmd_type) {
        ...
        case START_RECORD:
            ...
            for (int i=0; i<arr_len; i++){
                ...
                pitch_comp_arr[i] = 0;
                yaw_comp_arr[i] = 0;
                IMU_timestamp_arr[i] = 0;
                ToF0_arr[i] = 0;
                ToF1_arr[i] = 0;
                ToF0_timestamp_arr[i] = 0;
                ToF1_timestamp_arr[i] = 0;
            }
            IMU_arr_counter = 0;
            ToF0_arr_counter = 0;
            ToF1_arr_counter = 0;
...
void loop()
{
    ...
    if (central) {
        ...
        while (central.connected()) {
              ...
              if (start_record && IMU_arr_counter<arr_len) {
                  if (myICM.dataReady()) {
                      myICM.getAGMT();
                      ...
                      if (IMU_arr_counter==0) {
                          ...
                          // Get the current time in milliseconds
                          IMU_timestamp_arr[0] = (int) millis();
                      }
                      else {
                          // Calculate the angles from the measurements
                          // Accelerometer
                          ...
                          IMU_timestamp_arr[IMU_arr_counter] = (int) millis();
                          dt = IMU_timestamp_arr[IMU_arr_counter] - IMU_timestamp_arr[IMU_arr_counter-1];

                          roll_comp_arr[IMU_arr_counter] = (roll_comp_arr[IMU_arr_counter-1] + myICM.gyrY() * dt/1000)*0.7 + acc_roll_lp*0.3;
                          pitch_comp_arr[IMU_arr_counter] = (pitch_comp_arr[IMU_arr_counter-1] + myICM.gyrX() * dt/1000)*0.7 + acc_pitch_lp*0.3;
                          yaw_comp_arr[IMU_arr_counter] = yaw_comp_arr[IMU_arr_counter-1] + myICM.gyrZ() * dt/1000;
                      }

                      IMU_arr_counter += 1;
                      if (IMU_arr_counter == arr_len)
                          Serial.println("IMU recording finished.");
                      }
                      else {
                ...
                if (start_record && ToF0_arr_counter < arr_len && distanceSensor0.checkForDataReady()) {
                      ToF0_arr[ToF0_arr_counter] = distanceSensor0.getDistance();
                      ToF0_timestamp_arr[ToF0_arr_counter] = (int) millis();
                      ToF0_arr_counter += 1;
                  }
                
                if (start_record && ToF1_arr_counter < arr_len && distanceSensor1.checkForDataReady()) {
                      ToF1_arr[ToF1_arr_counter] = distanceSensor1.getDistance();
                      ToF1_timestamp_arr[ToF1_arr_counter] = (int) millis();
                      ToF1_arr_counter += 1;
                  } 
